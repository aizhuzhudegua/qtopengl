...
void MyOpenGLWidget::initializeGL()
{
    ...
    */
    ...
    m_postProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/shaders/vertex_shader.vert");
    m_postProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/shaders/post_processing.frag");
    ...
    // 指定纹理过滤缩小(MIN)/放大(MAG)采样方式
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // 将纹理绑定到m_fbo
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_texture, 0);

    // 创建渲染缓冲对象attachment，包含深度和模板缓冲
    ...
    // 这里将屏幕渲染的内容，以纹理方式渲染到小窗口中
    m_windowMesh=processMesh(transparentVertices,6,m_texture);
...
void MyOpenGLWidget::paintGL()
{
    ...
    */
    ...
    m_postProcessingShaderProgram.setUniformValue("view", m_view);
    m_postProcessingShaderProgram.setUniformValue("projection", m_projection);
    m_shaderProgram.setUniformValue("material.shininess", 32.0f);
    ...
    for(std::map<float,QVector3D>::reverse_iterator riter=sorted.rbegin();
        ...
        m_postProcessingShaderProgram.setUniformValue("model", m_model);

...
void MyOpenGLWidget::paintGL()
{
    ...
    case MyOpenGLWidget::Rect:
    {
        ...
        */
        ...
        // 设置光源的环境光强度较弱
        m_shaderProgram.setUniformValue("spotlight.ambient", QVector3D(0.4f, 0.4f, 0.4f));
        // 设置光源的光为较暗
        m_shaderProgram.setUniformValue("spotlight.diffuse", QVector3D(0.9f, 0.9f, 0.9f));
        // 设置镜面光为最大强度强度发光
        m_shaderProgram.setUniformValue("spotlight.specular", QVector3D(1.0,1.0,1.0));
        ...
        // 这里让光照位置与相机位置重合
        m_shaderProgram.setUniformValue("spotlight.position",  m_camera.m_cameraPos);
        // 这里聚光方向用了camera的方向代替了
        m_shaderProgram.setUniformValue("spotlight.spotDirection", m_camera.m_cameraLookAtFrontDirection);
        m_shaderProgram.setUniformValue("spotlight.cutoffAngle", (float)cos(12.5f*M_PI/180) );
        m_shaderProgram.setUniformValue("spotlight.outerCutoffAngle", (float)cos(17.5f*M_PI/180));
        ...
        // 定向光
        m_shaderProgram.setUniformValue("directionlight.direction", -0.2f, -1.0f, -0.3f);
        m_shaderProgram.setUniformValue("directionlight.ambient", 0.05f, 0.05f, 0.05f);
        m_shaderProgram.setUniformValue("directionlight.diffuse", 0.4f, 0.4f, 0.4f);
        m_shaderProgram.setUniformValue("directionlight.specular", 0.5f, 0.5f, 0.5f);

        for(int index = 0;index < 4;index++)
        {
            QString iStr="pointlights["+QString::number(index)+"].position";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), pointLightPositions[index].x(), pointLightPositions[index].y(), pointLightPositions[index].z());
            iStr="pointlights["+QString::number(index)+"].ambient";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), pointLightColors[index].x() * 0.1, pointLightColors[index].y() * 0.1, pointLightColors[index].z() * 0.1);
            iStr="pointlights["+QString::number(index)+"].diffuse";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), pointLightColors[index].x(), pointLightColors[index].y(), pointLightColors[index].z());
            iStr="pointlights["+QString::number(index)+"].specular";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), pointLightColors[index].x(), pointLightColors[index].y(), pointLightColors[index].z());
            iStr="pointlights["+QString::number(index)+"].constant";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), 1.0f);
            iStr="pointlights["+QString::number(index)+"].linear";
            m_shaderProgram.setUniformValue(iStr.toStdString().c_str(), 0.09f);
            iStr="pointlights["+QString::number(index)+"].quadratic";
